...
/**
 * The implementation of RestRequests interface which handles the communication
 * with the API. The execution uses apache's HTTPClient.
 *
 * @author kismoha
 */
public class RestRequestsImpl implements RestRequests {
    ...
    /**
     * This method handles the initial message which is used to create the maze
     * and get the starting cell and the maze ID. The path is BASE_URL/api/init.
     * POST method.
     *
    ...
    public APIResponse initMaze() throws HTTP400StatusException {
        try (CloseableHttpClient client = HttpClients.createDefault();) {
            ...
            return parseFromJson(executeMethod(client, post));
    ...
    /**
     * This method handles a movement in the maze to a neighbouring cell. The
     * path is BASE_URL/api/move. POST method.
     *
     ...
     */
    @Override
    public APIResponse moveTo(String mazeGuid, String direction)
            throws HTTP400StatusException {
        try (CloseableHttpClient client = HttpClients.createDefault();) {
     ...
     * already visited. The path is BASE_URL/api/jump. POST method.
     *
     ...
     */
    @Override
    public APIResponse jumpTo(String mazeGuid, int x, int y)
            throws HTTP400StatusException {
        try (CloseableHttpClient client = HttpClients.createDefault();) {
    ...
    /**
     * This method handles querying of the current cell. The path is
     * BASE_URL/api/currentCell.
     * GET method.
     *
    ...
    public APIResponse currentCell(String mazeGuid) {
        try (CloseableHttpClient client = HttpClients.createDefault();) {
            ...
            return parseFromJson(executeMethod(client, get));

        } catch (IOException | URISyntaxException ex) {
            throw new UnknownError("Error while erquesting current cell");
        }
    }

    // Executes a request to the API and returns the response json String
    private String executeMethod(CloseableHttpClient client,
            HttpUriRequest message) throws IOException {
